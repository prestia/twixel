...
Dynamic exactSize = __o_exactSize.Default(false);
	HX_STACK_PUSH("FlxBitmapDataPool::get","flixel/util/FlxBitmapDataPool.hx",41);
	HX_STACK_ARG(w,"w");
	HX_STACK_ARG(h,"h");
	...
	HX_STACK_ARG(exactSize,"exactSize");
{
		...
		while(((node != null()))){
			...
			HX_STACK_LINE(49)
			if (((bool((bool((bool((bmd->get_transparent() == transparent)) && bool((bmd->get_width() >= w)))) && bool((bmd->get_height() >= h)))) && bool(((bool(!(exactSize)) || bool((bool((bool(exactSize) && bool((bmd->get_width() == w)))) && bool((bmd->get_height() == h)))))))))){
				HX_STACK_LINE(52)
				res = bmd;
				HX_STACK_LINE(55)
				if (((node->prev != null()))){
...
{
		HX_STACK_PUSH("FlxBitmapDataPool::put","flixel/util/FlxBitmapDataPool.hx",89);
...
{
		HX_STACK_PUSH("FlxBitmapDataPool::clear","flixel/util/FlxBitmapDataPool.hx",121);
		HX_STACK_LINE(122)
		::flixel::util::_FlxBitmapDataPool::FlxBitmapDataPoolNode node = ::flixel::util::FlxBitmapDataPool_obj::_head;		HX_STACK_VAR(node,"node");
		HX_STACK_LINE(123)
		while(((node != null()))){
...
int FlxBitmapDataPool_obj::set_maxLength( int value){
	HX_STACK_PUSH("FlxBitmapDataPool::set_maxLength","flixel/util/FlxBitmapDataPool.hx",135);
...
void FlxBitmapDataPool_obj::__Mark(HX_MARK_PARAMS)
{
	HX_MARK_BEGIN_CLASS(FlxBitmapDataPool);
	HX_MARK_END_CLASS();
}

void FlxBitmapDataPool_obj::__Visit(HX_VISIT_PARAMS)
{
}
